# carbonhotkey-rs coverage audit v2 (vs MacOSX26.2.sdk)
...
Focused scope audit covering the `kVK_*` virtual keycodes from `Events.h` (120 constants) and the hotkey-registration API from `CarbonEvents.h` and `CarbonEventsCore.h` (19 symbols: `EventTypeSpec`, `EventHotKeyID`, `EventHotKeyRef`, `RegisterEventHotKey`, `UnregisterEventHotKey`, `GetApplicationEventTarget`, `InstallEventHandler`, `RemoveEventHandler`, `GetEventClass`, `GetEventKind`, `GetEventParameter`, plus event class/kind constants and error codes). This is the intentional API surface the crate exposes, not a full HIToolbox audit.
